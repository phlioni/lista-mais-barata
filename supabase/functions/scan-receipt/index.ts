...
serve(async (req) => {
    ...
    try {
        ...
        // Prompt do Sistema para o GPT-4o
...
2. Uma lista de itens esperados (JSON) que o usuário planejou comprar.
...
2. Se o produto for o mesmo (ex: Café) mas a marca for diferente (ex: Nota diz "CAF PELE", Lista diz "Café Pilão"), coloque em "review_needed".
3. Se o item da nota não existir na lista, coloque em "new_items".
4. Ignore itens que não sejam produtos (ex: "SUBTOTAL", "TROCO").
5. Preços devem ser números (float).
`;

        const userPrompt = `Aqui está a lista de itens que eu planejei comprar (use estes IDs para o retorno):
${JSON.stringify(currentItems, null, 2)}

Analise a imagem da nota fiscal e gere o JSON de conciliação.`;
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                model: 'gpt-4o', // Modelo Multimodal necessário
                messages: [
                    { role: 'system', content: systemPrompt },
                    {
                        ...
                        content: [
                            ...
                            { type: "image_url", image_url: { url: imageBase64 } } // Base64 direto
                        ]
                    }
                ],
                max_tokens: 2000,
                temperature: 0.1, // Baixa temperatura para ser preciso
